...
pub fn recommend_games_locally(
    ...
    AiRecommendationResponse {
        ...
        llm_used: false,
        diagnostic: Some("未配置 LLM Key 或未完成增强时，使用本地规则匹配和库内质量指标排序。".to_string()),
        items: ranked,
    }
}
...
fn quality_score(game: &GameCard) -> f64 {
    let base_score = game
        ...
        .clamp(0.0, 100.0);
    let review_score = game.positive_review_pct.unwrap_or(base_score).clamp(0.0, 100.0);
...
fn review_confidence(total_reviews: u32) -> f64 {
    ...
    (total_reviews as f64).log10().mul_add(20.0, 20.0).clamp(35.0, 100.0)
}
...
fn activity_score(current_players: u32) -> f64 {
    ...
    (current_players as f64).log10().mul_add(18.0, 35.0).clamp(35.0, 100.0)
}
...
fn build_reason(game: &GameCard, matched_traits: &[String], missing_traits: &[String]) -> String {
    ...
    format!("《{}》{}，可作为放宽条件后的备选。", game.name, quality_summary)
}
