...
M1 **不要求**在本仓库内完成 2,000 条真实采集写入；当时按官方目录路线判断需要 Key 与合规调度，属 M2。该历史判断不适用于后续新增的易变商店搜索候选适配器。
...
官方目录路线仍需 Steam Web API Key，可使用同一 `AppListRequest`/`AppListCursor` 连续拉页。M3 在 2026-07-14 另行验证了无需 Key 的 Steam 商店多人分类搜索：低频、可恢复采集得到 2,071 条真实候选。该来源只满足候选发现，不替代 `appdetails`、评价、CCU 与人工熟人联机画像。
