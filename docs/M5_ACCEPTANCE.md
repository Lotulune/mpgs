...
代码侧交付与离线退出条件由 `m5_acceptance.ps1` 门禁；脚本拒绝在 dirty 工作树上生成 PASS。生产 Key 联调仍为可选增强证据，但传入 `-LiveAi` 后必须完成真实请求并得到 `used/cached`。审查修复已在干净提交 `1a2188c` 上通过 `11/11` 离线门禁，记录见 [`M5_ACCEPTANCE_RUN.md`](M5_ACCEPTANCE_RUN.md)，M5 已重新关闭。
