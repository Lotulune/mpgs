...
当前状态：`MVP 0.1 / M3 验收完成、M4 进行中` — 生成式 OpenAPI、公开限流、版本化算法配置、完整偏好过滤和 SQLite 并发隔离均已接入；真实多人候选约 2,000+ 条并通过目录审计。数据富化已接入 `import-golden-profiles`（深度联机画像）与 `enrich-steam-candidates`（平台/评价/CCU 分批采集），覆盖率仍在抬升中；该数据门禁与 M4 客户端收口并行。
