...
当前状态：`MVP 0.1 / M3 验收完成、M4 可开始` — 生成式 OpenAPI、公开限流、版本化算法配置、完整偏好过滤和 SQLite 并发隔离均已接入；2026-07-14 使用 Steam 商店多人分类建立了 2,071 条真实候选并通过目录审计，GitHub Actions 的 Linux/Windows x64/ARM64 原生构建也全部通过。候选分类证据不等于深度多人画像，平台、评价、CCU 等字段仍需后续采集；该数据富化是发布门禁，但不阻塞 M4 客户端开发。
